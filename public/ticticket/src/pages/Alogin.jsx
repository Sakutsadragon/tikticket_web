...
import { aloginRoute } from '../utils/APIRoutes';
function ALogin() {
    ...
    const clisubmission= async(event)=>{
        ...
        if(validentry()){
            ...
            const {data}= await axios.post(aloginRoute,{
                username,
                password,
            });
            ...
            if(data.status === true){
                localStorage.setItem("ATikuser", JSON.stringify(data.userC));
                navigate("/ahomee");
            }
  ...
  return (
    ...
    <span>Don't have an account? <Link to="/aregister">Register</Link></span>
